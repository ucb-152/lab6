...
BINDIR=${GEMDIR}/software/gemmini-rocc-tests/build/
```
...
```bash
cd ${SIMDIR}
make CONFIG=GemminiRocketConfig run-binary BINARY=${BINDIR}/bareMetalC/template-baremetal
```
...
As you can tell, programming the accelerator requires not only knowledge of the programming language (in this case C), but also the special ways and instructions we must use to interact with the hardware accelerator, which we can consider as a "accelerator API". A large portion of ongoing development in research and industry is making the programming interface for accelerators easier, and some companies even develop plugins for their accelerator API in packages like PyTorch to allow developers to program directly in Python. However, lowering these high-level abstractions into the hardware API is still a work in progress, and getting the best performance will still involve direct programming in the hardware API.

> [!NOTE]
>
> Before proceeding, make sure to read the [Gemmini ISA overview](https://github.com/ucb-bar/gemmini/tree/master?tab=readme-ov-file#isa).
